...
int androidVersionMajor()
{
	const auto androidVersion    = QSysInfo::productVersion();
	auto majorVersion            = androidVersion;
	const auto decimalPointIndex = androidVersion.indexOf(".");
	if (decimalPointIndex > 0)
		majorVersion = androidVersion.left(decimalPointIndex);

	return majorVersion.toInt();
}

const auto ANDROID_VERSION_MAJOR = androidVersionMajor();
...
void ReadPasswordJobPrivate::scheduledStart()
{
    ...
    auto cipher = Cipher::getInstance(QStringLiteral("RSA/ECB/PKCS1Padding"), QStringLiteral("AndroidKeyStoreBCWorkaround")); // error in android 5: NoSuchProviderException: Provider not available: AndroidKeyStoreBCWorkaround
    if (ANDROID_VERSION_MAJOR < 6) { // below android m (6 Marshmallow)
        cipher = Cipher::getInstance(QStringLiteral("RSA/ECB/PKCS1Padding"), QStringLiteral("AndroidOpenSSL")); // error in android 6: InvalidKeyException: Need RSA private or public key
    }
